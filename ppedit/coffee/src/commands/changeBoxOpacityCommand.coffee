...
class ChangeBoxOpacityCommand extends Command
  ...
  changeOpacityToVal: (value) ->
    if @editPage
      ...
      @editor.panel1.element.find("tr[ppedit-box-id="+ @boxId + "]").find('.ppedit-slider').slider('setValue', value*100)
    else
      @editor.area2.boxesContainer.changeBoxOpacity @boxId, value
      @editor.panel2.element.find("tr[ppedit-box-id="+ @boxId + "]").find('.ppedit-slider').slider('setValue', value*100)
