...
class MainPanel extends Graphic
  ...
  bindEvents: ->
    ...
    @element.find('.snapImg').mouseover (event) =>
      $(event.target).attr('src', 'images/icons/ON/glyphicons_023_magnet.png')
    @element.find('.snapImg').mouseout (event) =>
      $(event.target).attr('src', 'images/icons/OFF/glyphicons_023_magnet.png')
    ...
    @element.find('.gridImg').mouseover (event) =>
      $(event.target).attr('src', 'images/icons/ON/glyphicons_155_show_big_thumbnails.png')
    @element.find('.gridImg').mouseout (event) =>
      $(event.target).attr('src', 'images/icons/OFF/glyphicons_155_show_big_thumbnails.png')

    @element.find(".undoImg").click =>
      @root.trigger 'requestUndo'

    @element.find('.undoImg').mouseover (event) =>
      $(event.target).attr('src', 'images/icons/ON/glyphicons_221_unshare.png')
    @element.find('.undoImg').mouseout (event) =>
      $(event.target).attr('src', 'images/icons/OFF/glyphicons_221_unshare.png')

    @element.find(".redoImg").click =>
      @root.trigger 'requestRedo'

    @element.find('.redoImg').mouseover (event) =>
      $(event.target).attr('src', 'images/icons/ON/glyphicons_222_share.png')
    @element.find('.redoImg').mouseout (event) =>
      $(event.target).attr('src', 'images/icons/OFF/glyphicons_222_share.png')